...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    "VLLM_DISABLE_FLASHINFER_PREFILL": lambda: (
        os.environ.get("VLLM_DISABLE_FLASHINFER_PREFILL", "0") == "1"
    ),
    ...
    # Deprecated, see profiler_config.
    "VLLM_TORCH_PROFILER_RECORD_SHAPES": lambda: os.getenv(
        "VLLM_TORCH_PROFILER_RECORD_SHAPES"
    ),
    # Enable torch profiler to profile memory if set to 1.
    # Deprecated, see profiler_config.
    "VLLM_TORCH_PROFILER_WITH_PROFILE_MEMORY": lambda: os.getenv(
        "VLLM_TORCH_PROFILER_WITH_PROFILE_MEMORY"
    ),
    # Enable torch profiler to profile stack if set to 1.
    # Deprecated, see profiler_config.
    "VLLM_TORCH_PROFILER_WITH_STACK": lambda: os.getenv(
        "VLLM_TORCH_PROFILER_WITH_STACK"
    ),
    # Enable torch profiler to profile flops if set to 1.
    # Deprecated, see profiler_config.
    "VLLM_TORCH_PROFILER_WITH_FLOPS": lambda: os.getenv(
        "VLLM_TORCH_PROFILER_WITH_FLOPS"
    ),
    # Disable torch profiling of the AsyncLLMEngine process if set to 1.
    # Deprecated, see profiler_config.
    "VLLM_TORCH_PROFILER_DISABLE_ASYNC_LLM": lambda: os.getenv(
    ...
    # Deprecated, see profiler_config.
    "VLLM_TORCH_PROFILER_DUMP_CUDA_TIME_TOTAL": lambda: os.getenv(
